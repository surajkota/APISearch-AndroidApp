    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ((DatumAdapter) m_adapter).setOnItemClickListener(new DatumAdapter.DatumClickListener() {
            ...
            public void onItemClick(int position, View v) {
                ...
                Intent intent_nxt= new Intent(API_Search.this, ResultDisplayActivity.class);
                intent_nxt.putExtra("PassCard", ((DatumAdapter) m_adapter).getItem(position));
                intent_nxt.putExtra("Position_in_Arraylist", position);
                startActivity(intent_nxt);
            }
            //On click of favourite button take the action of set/unset as favourite by checking it from database
            public void ontoggleClick(int position, View v) {
                Datum item = (Datum) ((DatumAdapter) m_adapter).getItem(position);
                boolean changestofav=false;
                if(!item.isFavourite(self))
                {
                    changestofav=true;
                }
                item.setFavourite(changestofav, self);
            }
        });
